...
Single static file containing every country **and dependent territory** with: ISO code, display name, `type` (`country` | `territory`), capital city, lat/lng, SVG centroid (`svgCx`/`svgCy`), world-map SVG path (`path`), flag path (`flag`), silhouette path (`shape`), region, `tier` (1–4), `langs` (ISO 639-1 codes of official languages), `subdivisions` (top-level admin divisions with `name` and `cat`), and `hasMapPath` / `hasShape` booleans.

- **`type`** — `country` for sovereign states, `territory` for dependent/special territories. The split is authoritative from `datasources/attribution.json` (its `countries` / `territories` sections). Both types are loaded into the atlas and are playable; The Knowledge page (`/knowledge`) has an All / Countries / Territories filter.
- **`flag`** — flags live under `public/flags/countries/<code>.svg` or `public/flags/territories/<code>.svg` (categorised by `type`). The `flag` field stores the path without the leading `/`. Always render flags via the `FlagImage` component (reads `atlas.flagPaths[code]`) rather than hardcoding a path.

`data.json` is regenerated by `node scripts/sync.mjs`, which chains the `scripts/add-*.mjs` and `scripts/categorize-countries.mjs` steps. `categorize-countries.mjs` stamps `type` + the categorised `flag` path from `attribution.json` and inserts any countries/territories missing from `data.json` (it skips the deprecated duplicate codes `bu`/`tp`/`an`). Silhouettes are fetched separately by `scripts/sync-silhouettes.mjs`.
